...
public function actionUsuariosPorPais() {
	...
	$consulta = Yii::app()->db->createCommand('SELECT p.id_pais, p.pais,
		...
		WHERE p.id_pais = p2.id_pais) AS cantidad
		FROM pais p
...
$sql = "SELECT to_char(date_trunc('day', fecha_creacion), 'DD/MM/YY') AS fecha,
...
$data[] = Yii::app()->db->createCommand("select count(1) from usuario")->queryScalar();
$data[] = Tweet::model()->count() ;
$data[] = Retweet::model()->count() ;
$data[] = Favorito::model()->count() ;
